...
You should delegate running bash commands (particularly ones that are likely to produce lots of output) such as investigating with the `gcloud` CLI, using the `gh` CLI, digging through logs to `Bash` sub-agents. 
